...
module.exports = function(templateSettings) {
    ...
    templateSettings.imports['includeScenario'] = includeScenario;
...
function includeScenario(filename) {
    var content = readFile(filename);
    content = content.replace(/^\$schema:(.*)$/m, '');
...
function schema2md(filename) {
    ...
    var propertiesArr = _.extend({}, content.properties, content.patternProperties);
    if (_.keys(propertiesArr).length) {
        _.forEach(propertiesArr, function (property, name) {
            if (name === '$schema') {
                return;
            }
    ...
    properties.forEach(function(property) {
        output += '| ' + property.name + ' |' +
            ...
            (property.default !== undefined ? ' __Default:__ ' + property.default : '' ) +
            ' |' +
